...
public class HistorySummaryDto {
    ...
    public static HistorySummaryDto of(List<RoutineHistory> histories) {
        int totalWorkoutTime = histories.stream()
                .mapToInt(h -> h.getTotalWorkoutTime())
                .sum();

        double totalWeights = histories.stream()
                .mapToDouble(h -> h.getTotalWeightLifted())
                .sum();

        return HistorySummaryDto.builder()
                .totalWorkoutTime(totalWorkoutTime)
                .totalworkoutWeight(totalWeights)
                .build();
    }
}
